---
title: イリア（上位物語）
layout: default
permalink: /staff/ilya/ja
lang: ja
---

# イリア（上位物語）・プロフィールカード

<span lang="zh" style="font-family: 'Noto Sans SC'">[中文](/staff/ilya)</span> | [English](/staff/ilya/en) | **日本語**

## 基本情報

**名前：**イリア（Ilya）<br>
**あだ名：**ソーミル（Sawmill）<br>
**学年：**高校3年生予定<br>
**タイムゾーン：**UTC+8、*5年後に大阪（UTC+9）へ移住予定*<br>
**MBTI：**ENTP<br>
**メールアドレス：**[ilya@lokova.org](mailto:ilya@lokova.org)<br>
**SNS：**

* GitHub：[@IlyaTheSawmill](https://github.com/IlyaTheSawmill)
* Discord：[@ilya_yezelovsky](https://discord.com/users/1216215110960283728)
* X：[@ilya_sawmill](https://x.com/ilya_sawmill)
* Bilibili：[@伊利亚锯木机](https://space.bilibili.com/3493084490893445)
* QQ：1026491165
* Steam：[@ilya_yezelovsky](https://steamcommunity.com/id/ilya_yezelovsky/)

## 趣味・興味

### ゲーム

国際版しか遊ばない。中国版は遊ばない。

PCゲームしか遊ばない。スマホゲームはやらない。

**Tencent・NetEaseのスマホゲームは地雷。**それ以外（*原神、崩壊：スターレイル、アークナイツ、鳴潮など*）は好きでも嫌いでもない。プレイしたこともない。

#### Minecraft

ガチ勢。Java版（国際版）のみプレイ（プレイヤー名：[`IlyaTheSawmill`](https://mineatar.io/player/IlyaTheSawmill)）。Bedrock版・NetEase版はやらない。

主なプレイスタイル：サバイバル、レッドストーン、自鯖開発。**バニラは受け付けない。X-ray系チート必須。**

Minecraftスタイルについてもっと知りたい場合は、中国語版Minecraft Wikiの私のユーザーページを見ることができる：[IlyaYezelovsky](https://zh.minecraft.wiki/w/User:IlyaYezelovsky)

2019年5月18日にMinecraftを始めた。

#### Steam

ライトユーザー。**AAAタイトルは遊ばない。**小規模・インディー系が中心。現在はRusted Warfareをプレイ中。

#### ギャルゲー

**つい最近（2026年5月5日）始めたばかりで、まだ1作品しかプレイしていない。**

[SMEE](https://vndb.org/p328)の[Making＊Lovers](https://vndb.org/v21552)を遊んで、自分の好みがはっきり分かった。

純愛・イチャイチャ・糖度MAX・日常コメディ、いわゆる「萌えゲー」が好き。

今後もこの系統を中心に遊ぶ予定。**鬱ゲーには手を出すつもりはない。**

### 技術

#### Java

*Minecraftのおかげで覚えた（草）*

たまに小さなツールを作っている。

現在開発中：[OCWorkshop](https://github.com/IlyaTheSawmill/OCWorkshop)。OC創作支援ツール。

#### Minecraftサーバー

自分でプラグインサバイバルサーバー（[LokovaMC](https://mc.lokova.org/)）を運営している。**ただし、あまり開いていない。** *宣伝（無償）：[Leaves](https://leavesmc.org/)サーバーソフト最高！*

### アニメ

**ライト勢。追わない。ファンダムには入らない。BL・百合は見ない。V界隈お断り。**たまたま気になる作品があれば見る程度。

### 創作

自作世界観あり（[Lokova](https://www.lokova.org/)、[設定紹介ページ](https://www.lokova.org/fic)）*ページは中国語*。「Z世代だけの都市」がテーマ。

設定の矛盾探しやアイデア提供は大歓迎。ちゃんと読んだうえでツッコんでくれる人は特に歓迎。

### 鉄オタ

主に日本の鉄道。中国の鉄道の話もできる。

## フレンド追加について

* 普段はテキスト派。マルチプレイ中なら音声通話（[Simple Voice Chat](https://modrinth.com/plugin/simple-voice-chat)など）もOK。**中国語・英語のみ対応**。
* 返信速度：見たら必ず返す。ただしPCの前にいないとかなり遅い。**PCがオンライン＝本当にオンライン。スマホだけならほぼオフライン。**
* フレンド申請時は一言して。何も書いていない場合は見落とすかもしれない。

## 地雷

* **推し活・アイドル界隈：全部。何もかも。例外なし。** 中国・韓国・日本・欧米問わず、箱推し・単推し・CP厨・過激派……**全部お断り。**芸能人は追わないし、語らない。推しの布教もしないでほしいし、ここで揉めるのもNG。**最大の地雷。踏んだら即ブロック。**
* **スマホゲーム：Tencent・NetEaseのスマホゲームは地雷。**その他に気にしない。
* **Minecraft：**バニラ原理主義の人は合わないと思う。
* **アニメ：**アニメは追わない、ファンダムに入らない、BL・百合は見ない、V界隈お断り。
* **ギャルゲー：**萌えゲー特化。鬱ゲー・シナリオ重視派とは合わないと思う。
* **技術：**プログラミング言語論争はしない。~~Java最強、異論は認めない。~~
* **キーボード政治：**理性的な議論は歓迎。**過激な人はお断り。**

## 加点ポイント

* Minecraft Java版でプラグイン・Mod・レッドストーン・自鯖運営をやっている
* 自分の世界観サイトを読んで、しっかり感想やツッコミをくれる
* 日本の鉄道が好き
* 自分の変な発想に付き合える

> **趣味が合う人優先。ネガティブすぎる人や、尋問みたいな会話をする人は不似合い。追加する前に地雷を確認してお互い平和に。**

***最終更新：2026年6月30日（火）***